...
public class RejectRequest extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String sql = "UPDATE requests SET status = 'Rejected' WHERE req_id =" + req_id;
            stmt.executeUpdate(sql);
            PrintWriter out = response.getWriter();
//            out.print("@rejrequest");
            request.getRequestDispatcher("PenReq.jsp").forward(request, response);
        } catch (SQLException ex) {
            Logger.getLogger(RejectRequest.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
